  ...
  public:
            ...
            //ZRenderer re(this) = m_renderer;
             ...
             for(std::list<ZNet*>::iterator i=nets.begin();i!=nets.end();++i) {
                 ...
                 for (j = terms.begin(); j != terms.end(); ++j) {
                     ...
                     SDL_Log("%s %d %d",(*j)->net()->get_name().c_str(),col_to_x((*j)->col()),row_to_y((*j)->row()));
        ...
        bool is_in_term_bounds( int a ,  int  b) {
         ...
         void zoom_in() {
             ...
             m_render_const_factor --;
             m_render_const_delta_x --;
             m_render_const_delta_y --;
                        
             m_term_shape_rect_size --;
			 m_highlight_thickness --;
             m_extension_point_radius --;
			 //dump_data();
         }
...
/*
		...
		virtual void zoomin_to_point(float x,float y) {
			m_render_const_delta_x = x/10;
			m_render_const_delta_y = y/10;
			zoom_in();
 ...
 private:
		...
		ZColor get_net_color(ZNet* n) {
			...
			ZColor z_color;
                        ...
                        z_color.r = m_net2color[n].r/10;
                        z_color.g = m_net2color[n].g/10;
                        z_color.b = m_net2color[n].b/10;
